...
THREE.FirstPersonControls = function ( object, domElement ) {
	...
	this.onMouseMove = function ( event ) {
		...
		if ( this.domElement === document ) {
			...
			this.mouseX = event.pageX - ( boundingClientRect.left + ( boundingClientRect.width / 2 ) )
			this.mousey = event.pageY - ( boundingClientRect.top + ( boundingClientRect.height / 2 ) )

		}

		if ( Math.abs(this.mouseX) < this.epsilon ) {
			this.mouseX = 0;
		}

		if ( Math.abs(this.mouseY) < this.epsilon ) {
			this.mouseY = 0;
		}
